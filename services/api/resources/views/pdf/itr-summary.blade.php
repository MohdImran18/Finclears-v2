...
<td>{{ optional($return->assessmentYear)->name ?? "-" }}</td>
...
<td>{{ optional($return->financialYear)->name ?? "-" }}</td>
...
<td>{{ optional($return->returnStatus)->name ?? "-" }}</td>
...
<td>{{ optional($return->itrType)->name ?? "-" }}</td>
...
<td class="amount">
₹ {{ number_format($return->gross_income ?? 0,2) }}
</td>
</tr>
...
<td>
{{ optional($return->taxRegime)->name ?? "-" }}
</td>
...
<td class="amount">
₹ {{ number_format($return->tax_liability ?? 0,2) }}
</td>
...
<td class="amount">
₹ {{ number_format($return->net_payable ?? 0,2) }}
</td>
