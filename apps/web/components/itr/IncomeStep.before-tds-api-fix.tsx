...
const annual_salary = watch("annual_salary");
const exempt_allowances = watch("exempt_allowances");
const professional_tax = watch("professional_tax");

const rental_income = watch("rental_income");
const municipal_tax = watch("municipal_tax");
const home_loan_interest = watch("home_loan_interest");

const business_income = watch("business_income");
const business_expenses = watch("business_expenses");

const short_term_capital_gain = watch("short_term_capital_gain");
const long_term_capital_gain = watch("long_term_capital_gain");

const interest_income = watch("interest_income");
const dividend_income = watch("dividend_income");
const other_income = watch("other_income");
...
useEffect(() => {
    ...
    const currentIncome = useItrStore.getState().income;

    if (
        currentIncome.salary !== newIncome.salary ||
        currentIncome.houseProperty !== newIncome.houseProperty ||
        currentIncome.otherSources !== newIncome.otherSources ||
        currentIncome.grossIncome !== newIncome.grossIncome
    ) {
        setIncome(newIncome);
    }
}, [
    ...
    <Field
        ...
        error={errors.exempt_allowances?.message}
    >
        <Input
            type="number"
            {...register("exempt_allowances", {
                valueAsNumber: true,
            })}
        />
    </Field>
    ...
    <Field
        label="TDS Deducted"
        error={errors.tds_amount?.message}
    >
        <Input
            type="number"
            min="0"
            ...
            <Card>
                ...
                <CardContent className="grid gap-5 md:grid-cols-2">
                    ...
                    <Field
                        label="Municipal Tax"
                        error={
                            errors.municipal_tax?.message
                        }
                    >
                        <Input
                            type="number"
                            {...register("municipal_tax", { valueAsNumber: true })}
                        />
                    </Field>
            ...
            <Card>
                ...
                <CardContent className="grid gap-5 md:grid-cols-2">
                    ...
                    >
                        <Input
                            type="number"
                            {...register("short_term_capital_gain", { valueAsNumber: true })}
                        />
                    </Field>
                    ...
                    >
                        <Input
                            type="number"
                            {...register("long_term_capital_gain", { valueAsNumber: true })}
                        />
                    </Field>
                </CardContent>
            </Card>
            ...
            <Card>
                ...
                <CardContent className="grid gap-5 md:grid-cols-2">
                    ...
                    >
                        <Input
                            type="number"
                            {...register("interest_income", { valueAsNumber: true })}
                        />
                    </Field>
                    ...
                    >
                        <Input
                            type="number"
                            {...register("dividend_income", { valueAsNumber: true })}
                        />
                    </Field>
