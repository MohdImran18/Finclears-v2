...
function getActionIcon(action: TimelineAction) {
	switch (action) {
		...
		case "under_review":
			return <Search className="h-5 w-5 text-yellow-600" />;
		...
		default:
			return <Clock3 className="h-5 w-5 text-slate-600" />;
	}
}

function getActionTitle(action: TimelineAction) {
	switch (action) {
		case "uploaded":
...
export default function DocumentTimeline({ items = [] }: Props) {
	if (!items.length) {
		return (
			<div className="rounded-2xl border bg-white p-8 shadow-sm">
				...
				<div className="rounded-xl border border-dashed p-10 text-center">
					...
					<p className="font-medium">No activity available.</p>
				</div>
			</div>
		);
	}
	...
	return (
		<div className="rounded-2xl border bg-white p-6 shadow-sm">
			<h2 className="mb-8 text-xl font-semibold">Activity Timeline</h2>
						...
						<div key={item.id} className="relative flex gap-5">
							...
							<div className="flex-1 rounded-xl border bg-slate-50 p-5">
								...
								<p className="mt-2 text-sm text-slate-600">
									Performed by
									<span className="ml-1 font-semibold">{item.user}</span>
								</p>
