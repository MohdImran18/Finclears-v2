...
export default function UserTable({ users, meta, loading, error }: Props) {
	if (loading) {
		return (
			<div className="rounded-xl border bg-white p-6">Loading users...</div>
		);
	}
						...
						<tr key={user.id} className="border-t">
							...
							<td className="px-6 py-4">
								<div className="flex gap-2">
									<UserActions id={user.id} name={user.name} />
								</div>
							</td>
						</tr>
					))}
