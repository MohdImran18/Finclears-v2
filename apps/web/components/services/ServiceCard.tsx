...
export default function ServiceCard({ service }: ServiceCardProps) {
	const image =
		service.featured_image || "/images/services/service-placeholder.webp";

	const category = service.category?.name ?? "Business Service";

	const categorySlug = service.category?.slug ?? "general";

	const price = Number(service.starting_price ?? 0);

	const processingTime = service.processing_time ?? "2–5 Working Days";
